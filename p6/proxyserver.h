...
void parse_client_request(int client_fd, HTTPRequest *req);
...
void parse_client_request(int client_fd, HTTPRequest *req)
{
    ...
    char *read_buffer = malloc(LIBHTTP_REQUEST_MAX_SIZE + 1); //+1

    if (!read_buffer)
        http_fatal_error("Malloc failed");

    int bytes_read = read(client_fd, read_buffer, LIBHTTP_REQUEST_MAX_SIZE);
    ...
    // strcpy(req.read_buffer, read_buffer);
    strcpy(req->read_buffer, read_buffer);

    printf("In parse client request --- read buffer %s\n\n", read_buffer);
